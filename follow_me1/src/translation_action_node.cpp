...
translation_action() {
    ...
    // communication with amcletry
    //sub_amcl = n.subscribe("amcl_now", 1, &translation_action::amclCallback, this);
    sub_amcl = n.subscribe("odom", 1, &translation_action::amclCallback, this);
    ...
    obstacle_pub = n.advertise<std_msgs::Float32>("obstacle", 1);
...
void update() {
    ...
    //ROS_INFO("new_amcl: %i, cond_translation: %i, init_obstacle: %i", new_amcl, cond_translation, init_obstacle);
             // we receive a new /translation_to_do
    if ( new_translation_to_do && new_amcl ) {
        new_translation_to_do = false;
    ...
    if ( new_amcl && cond_translation && init_obstacle ) {
...
void amclCallback(const nav_msgs::Odometry::ConstPtr& o) {
    //ROS_INFO("(translation_action_node) received amcl");
    new_amcl = true;
...
// process the translation to do received from the decision node
    ROS_INFO("(translation_action_node) received translation_to_do");
