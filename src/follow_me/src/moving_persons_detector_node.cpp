...
void detect_motion() {
    ...
    for (int loop=0; loop<nb_beams; loop++ )//loop over all the hits
        dynamic[loop] = fabs(background[loop] - range[loop]) > detection_threshold;//the current hit is dynamic
...
//else we start a new cluster with the current hit and end the current cluster
    ...
    for( int loop=1; loop<nb_beams; loop++ ) {//loop over all the hits
        float distance = distancePoints(current_scan[loop], current_scan[loop-1]);
        ...
        if(distance < cluster_threshold) {
            ...
            cluster_dynamic[nb_cluster] = ((float)nb_dynamic / (float)(cluster_end[nb_cluster] - cluster_start[nb_cluster])) * 100.;
            ...
            ROS_INFO("cluster[%i]: (%f, %f) -> (%f, %f), size: %f,  middle: %f %f, dynamic: %i", 
                nb_cluster, current_scan[cluster_start[nb_cluster]].x, current_scan[cluster_start[nb_cluster]].y, 
    ...
    cluster_size[nb_cluster] = distancePoints(current_scan[cluster_start[nb_cluster]], current_scan[cluster_end[nb_cluster]]);
    cluster_dynamic[nb_cluster] = ((float)nb_dynamic / (float)cluster_size[nb_cluster]) * 100.;
    cluster_middle[nb_cluster].x = (current_scan[cluster_start[nb_cluster]].x + current_scan[cluster_end[nb_cluster]].x) / 2.;
    cluster_middle[nb_cluster].y = (current_scan[cluster_start[nb_cluster]].y + current_scan[cluster_end[nb_cluster]].y) / 2.;
...
// - more than "dynamic_threshold"% of its hits are dynamic (see, cluster_dynamic table)
    ...
    for (int loop=0; loop<nb_cluster; loop++){//loop over all the clusters
        if (cluster_size[loop] > leg_size_min && cluster_size[loop] < leg_size_max
            && cluster_dynamic[loop] > dynamic_threshold){
            ...
            //~ and lower than "leg_size_max" and it has "dynamic_threshold"% of its hits that are dynamic
        ...
        for (int loop_leg2 = loop_leg1 + 1; loop_leg2 < nb_moving_legs_detected; loop_leg2++){ //loop over all the legs
            if (distancePoints(moving_leg_detected[loop_leg1], moving_leg_detected[loop_leg2]) < legs_distance_max){
                ...
                moving_persons_detected[nb_moving_persons_detected].x = 
                    (moving_leg_detected[loop_leg1].x + moving_leg_detected[loop_leg2].x) / 2.;
                moving_persons_detected[nb_moving_persons_detected].y = 
                    (moving_leg_detected[loop_leg1].y + moving_leg_detected[loop_leg2].y) / 2.;
                        

                // textual displayng
                ROS_INFO("moving person detected[%i]: leg[%i]+leg[%i] -> (%f, %f)", 
                    nb_moving_persons_detected, loop_leg1, loop_leg2, moving_persons_detected[nb_moving_persons_detected].x, moving_persons_detected[nb_moving_persons_detected].y);
